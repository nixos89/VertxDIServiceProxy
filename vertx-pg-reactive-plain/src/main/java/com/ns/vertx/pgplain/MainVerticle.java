...
public class MainVerticle extends AbstractVerticle {
	...
	public void start(Promise<Void> startPromise) throws Exception {				
		...
		retriever.getConfig(h -> {
			if (h.succeeded()) {
				JsonObject hResult = h.result();
//				LOGGER.info(" ======== hResult ========= " + hResult.encodePrettily() + " =================");
			} else {
				LOGGER.error("Somethin' 's fucked up!");
			}
		});
		
		PgConnectOptions connectOptions = new PgConnectOptions()
				.setPort(5432).setHost("localhost")
				.setDatabase("vertx-pg-plain-db")
		...
		Future<Void> futureConnection = connect().compose(connection -> {
			Future<Void> retFuture = Future.future(); // TODO: change it later to Promise when everything works!
		...
		futureConnection.compose(v -> createHttpServer(pgClient, routerAPI))
